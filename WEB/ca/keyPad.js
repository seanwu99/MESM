$(document).ready(function () {
    ...
    $.extend($.keyboard.keyaction, {
        ...
        login: function (base) {
            ...
            // base.$keyboard.find('.ui-keyboard-rad').addClass(base.options.css.buttonAction);
            // base.$keyboard.find('.ui-keyboard-deg, .ui-keyboard-grad').removeClass(base.options.css.buttonAction);
            // console.log(base.memory2);
            base.memory2 = '';
            // console.log('login', base.checkCombos());
            // base.originalContent = base.checkCombos();
            // console.log('originalContent', base.originalContent);

            base.$preview.val(base.checkCombos() + 'ABC');
            base.close(true);
            // base.$preview.val('login');
        },
        // deg: function (base) {
        ...
        logout: function (base) {
            ...
            // cc(base.memory2);
            base.memory2 = '';
            base.$preview.val(base.checkCombos() + 'BBC');
            base.close(true);
        ...
        mplus: function (base) {
            if (base.memory === '') {
                ...
                base.$preview.val(function (i, v) {
                    return (v === '') ? '' : parseFloat(v) + parseFloat(base.memory);
                });
            }
        },
        'm-': function (base) {
        ...
        acosh: function (base) { // if(v>=1) return Math.log(v+Math.sqrt(v*v-1))
            base.memory2 = '';
            base.$preview.val(function (i, v) {
                return (v >= 1) ? Math.log(v + Math.sqrt(v * v - 1)) : v;
            });
        },
        ...
        atanh: function (base) { // if(v>1) return Math.log((1+v)/(1-v))/2
            base.memory2 = '';
            base.$preview.val(function (i, v) {
                return (v > 1) ? Math.log((1 + v) / (1 - v)) / 2 : v;
            });
        },
